...
export default function RecipeCreate() {
  ...
  const handleChange = (e) => {
    if (recipes.filter(r => r.title.toLowerCase() === input.title.toLowerCase()).length) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (input.image === "") {
        input.image = "https://storage.googleapis.com/diariodemocracia/news/Asado-99a56c.jpg"
    }
    ...
    <Body>
        ...
        <form onSubmit={e => handleSubmit(e)}>
            ...
            <button
                type="submit"
                disabled={
                    !input.title || !input.summary || errors.score || !input.healthscore || !input.diets.length ? true : false
                }>¡Crear Receta!
            </button>
